...
class Task:
    ...
    def from_dataset_info(json_obj: t.Dict) -> "Task":
        ...
        to identify the task type. This will be removed at some point in time in the future, and is only needed to
        process artifacts generated by earlier versions of th `xtime.training`.
        """
        try:
            from xtime.datasets.dataset import DatasetMetadata

            metadata = DatasetMetadata.from_json(json_obj)
            return metadata.task
        except (KeyError, ValueError):
            ...

        if "Dataset" in json_obj and "task" in json_obj["Dataset"]:
            task_type = json_obj["Dataset"]["task"]
            if task_type == "BINARY_CLASSIFICATION":
                return ClassificationTask(
                    TaskType.BINARY_CLASSIFICATION, num_classes=json_obj["Dataset"]["num_classes"]
                )
            if task_type == "MULTI_CLASS_CLASSIFICATION":
                return ClassificationTask(
                    TaskType.MULTI_CLASS_CLASSIFICATION, num_classes=json_obj["Dataset"]["num_classes"]
                )

        raise ValueError(f"Unsupported dataset info object ({json_obj}).")
