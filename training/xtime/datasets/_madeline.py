...
class MADELINEBuilder(DatasetBuilder):
    ...
    def _build_default_dataset(self, **kwargs) -> Dataset:
        ...
        train_df = pd.read_csv(self._dataset_dir / (_MADELINE_DATASET_FILE[0:-4] + "-default-train.csv"))
        test_df = pd.read_csv(self._dataset_dir / (_MADELINE_DATASET_FILE[0:-4] + "-default-test.csv"))

        # Features values are Integers, Labels: Binary Classification
        features = [
            Feature(
                col,
                FeatureType.BINARY if col == "label" else FeatureType.ORDINAL,
                cardinality=int(train_df[col].nunique()),
            )
            for col in train_df.columns
        ]

        # Check that data frames contains expected columns (259 Features: V1 to V259, 1 is for label).
        assert train_df.shape[1] == len(features), "Train data frame contains wrong number of columns."
        assert test_df.shape[1] == len(features), "Test data frame contains wrong number of columns."
    ...
    def _create_default_dataset(self) -> None:
        ...
        default_train_dataset_file = self._dataset_dir / (_MADELINE_DATASET_FILE[0:-4] + "-default-train.csv")
        default_test_dataset_file = self._dataset_dir / (_MADELINE_DATASET_FILE[0:-4] + "-default-test.csv")
